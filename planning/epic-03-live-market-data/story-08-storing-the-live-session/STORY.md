...
**Status:** **Complete — 2026-09-24. Ten of ten tasks.** Nine criteria, nine verdicts, and one of them — criterion 1 — is _met in the tree and unwitnessed in production_ rather than a plain yes, because every regular-session minute the writer could have written is shadowed by the consolidated bar the read prefers. The story's headline was read off production on the night of 2026-09-23: **the overnight reconciliation ran for real and the served answer names one source**, exactly as Task 3.8.9's rehearsal predicted. The close also found a narrowing nobody had written down — **the two-feed source note is a mid-session state on a deployed store**, not a standing one — and corrected the live IEX coverage figure in four more files, the third time this epic has paid for a correction that was recorded and not propagated. `LIVE-REHEARSAL.md`'s 3.8 row is **empty and owed**: the market is shut, and both items need it open. Previously: **nine of TEN tasks done (3.8.1–3.8.9)**, the last of them on 2026-09-23. The deployed backend is a bar writer, the two-feed source note is on screen, and a reconciled session is a chart rather than a 500. Originally **split into eight tasks 2026-09-23**; a ninth was inserted at 3.8.4 that day, and a **tenth at 3.8.5** on the same day with everything after it renumbered — Task 3.8.4 repaired one read that assumed one row a minute and the audit it prompted found a second, `readLastCloses`, which does not throw and whose wrong answer has no tell — latent today, because nothing calls it at a timeframe that can hold two tapes, and waiting directly in Task 3.8.8's path. See _Tasks_ below. The visible payoff is Task 3.8.3, which is as early as the two decisions before it allow.
...
| 3.8.5  | [The last close that is the same minute twice](TASK-05-the-last-close-that-is-the-same-minute-twice.md)                        | 3.8.4      | No — a latent lie closed — **done**         |
...
| 3.8.10 | [The sweep, the hand-offs and the close](TASK-10-the-sweep-the-hand-offs-and-the-close.md)                                     | 3.8.9      | No — **done**                               |
