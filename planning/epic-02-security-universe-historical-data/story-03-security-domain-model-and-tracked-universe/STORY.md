...
- **2.3.7** inherits one new fact and it is the one that would be discovered the expensive
  way: **the deployed `securities` table can now hold rows the file does not contain**, so
  the first deployed run of `pnpm universe` after any future removal writes an `update`
  rather than only inserts, and the row count and the universe count stop being the same
  number. Nothing about the step changes; what changes is what its output means.
- **2.3.8** gains `load-universe.ts`'s new `untrackAbsent` and the `README.md` paragraph
  ...
  **`pnpm test` is 287 and `pnpm test:database` is 55**. It also inherits the one live
  ...
  `UNIVERSE.md` §12.2, which is a gap of the third kind by construction.
