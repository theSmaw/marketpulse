...
> **Amended 2026-09-15 by Task 2.14.6, which built it — three things shipping
> it settled.**
>
> - **The volume headline is `No volume history stored for NVDA yet.`**, one
>   word longer than the amendment above specifies, and the word was bought
>   rather than chosen. `pnpm invariants` anchors on a literal a grep can find,
>   and the two case-one headlines interpolate the symbol — so each is anchored
>   on its longest fixed fragment. `No volume stored for ` is a **prefix of the
>   window sentence**, which would leave the check unable to tell the two homes
>   apart and green with the sentence deleted: the rot `CLAUDE.md` names, built
>   in on purpose. `No volume history stored for ` is distinct, and it reads as
>   the parallel of the price headline rather than as a hedge. Both drawn
>   headlines now match their spoken counterparts in subject and differ in
>   wording, which is the standing rule for the two channels.
> - **The `1m` seam is visible from the other side, and the wording already
>   covers it.** Removing one security's minute ledger row on a store that holds
>   its daily bars draws _No history stored for ZTS yet_ under an identity block
>   still printing a last close — because that close is a **daily** bar the
>   coverage array never described. The sentence stays true because it says _at
>   this timeframe_. That clause was §6.3's and it is now load-bearing rather
>   than careful; it is in [`docs/GAPS.md`](../../../docs/GAPS.md) with a
>   re-measure.
> - **The browser suite had to be taught the pair, and it would have gone red in
>   CI on the first push.** Nine specs locate a settled answer by the window
>   sentence, and **CI's store is 518 securities and zero bars** — so every
>   chart there becomes **case one** the moment this ships, and every one of
>   those locators would have matched nothing. They now read one exported
>   pattern, `AN_EMPTY_PLOT` in `e2e/support/app.ts`. One spec needed more than
>   a pattern: `security-series-states.spec.ts` forces an empty body and then
>   asserts the _window_ detail line, which case one does not draw, so it
>   branches on which answer is on screen and asserts both.
...
| the two vacancy sentences (§6)       | `apps/frontend/src/components/PriceChart/ChartVacancy.tsx`              | `pnpm invariants`, extended to all **four** literals, with a `pnpm break` entry each    |
| which empty answer it is (§6.2)      | `apps/frontend/src/components/PriceChart/chart-vacancy.ts`              | a pure function over the two views, unit-testable with no DOM — `source-note.ts`' shape |
...
| `VolumeReading` — `No shares changed hands anywhere in the window.`                                                                                                                                                                                                       | ~~**Read, left, and given a trigger.**~~ — **the trigger fired, and it was discharged on 2026-09-24 by Task 3.9.8.** The one shipped sentence claiming something about **the market** rather than about our store. True while every bar is the consolidated tape; a single venue's silence the moment Epic 3 stitches an IEX tail — which Story 3.8 made ordinary rather than theoretical. It now has **one home**, `describeSilence`, and names its own reach: `anywhere` on the consolidated tape, **the venue** where one venue is all the window holds, `on either feed` across a stitch. A check and a break stand behind the single home.                                                   |
...
| **`ChartVacancy` — `No history stored for NVDA yet.` and `No volume history stored for NVDA yet.`**, with `We hold no bars for this security at this timeframe. Changing the window will not help — the store is filled overnight.` **(added 2026-09-15 by Task 2.14.6)** | **New, and judged in both directions.** (a) It claims no coverage: its whole content is an absence in _our store_, and the subject of every clause is `we`. (b) It disclaims none either — and this is the row where (b) was the live risk, because _no history stored_ is the most disclaiming shape of sentence in the product. What keeps it exact is **`at this timeframe`**: the derivation reads the `1m` half of the ledger, so _this security holds nothing_ would be a claim wider than the evidence. The sentence says what was read.                                                                                                                                                   |
| **The spoken twins — `No history is stored for NVDA at this timeframe`, `No volume history is stored for NVDA at this timeframe`, and `no history is stored for this security at this timeframe`**                                                                        | **New, and read separately**, because §11's own lesson is that a pass which skips the spoken copy skips half the product. Same judgement in both directions, plus one thing only the spoken channel has: the announcement says _this security_ rather than the symbol, because every announcement already opens with the symbol and a listener hearing it twice in one breath is being told the subject twice.                                                                                                                                                                                                                                                                                    |
...
| **`VolumeChart` — `No volume to draw. The Price region says why.` (added 2026-09-15 by Task 2.14.7)**                                                                                                                                                                     | **New, and judged in both directions — and it is the first string in the product that is a _deferral_ rather than an answer.** (a) It claims no coverage: its subject is this plot and its content is that the account is elsewhere. (b) It disclaims none — and this is the row where (b) had to be argued, because _no volume to draw_ is the most disclaiming shape of sentence in the product. What keeps it exact is that it says nothing about the store at all: the series was refused or never arrived, so **we do not know what we hold**, and the sentence carefully does not guess. Compare `ChartVacancy`'s four, which say what _is_ stored; this one says only where the answer is. |
| **The three search hints, reworded (added 2026-09-15 by Task 2.14.7)** — `Search comes back when it does`, `Search comes back when it answers`, `asking again will not bring it back`                                                                                     | **New, and read in both directions, and neither fires** — these are claims about _search's availability_, not about venues. What they were read hardest for is a different criterion: whether each still honours `FRONTEND-STATE.md` §4 in words after the tracked universe's prospect was taken out of them. It does, and the asymmetry is the rule working — the permanent failure keeps its own prospect because there is no control to defer to, and the other two say _search comes back_, which is §4's _waiting may help_ about search's own subject.                                                                                                                                      |
...
> **Amended 2026-09-15 by Task 2.14.7, which closes this table.** Task 2.14.5's
> own record asked that the pass cover the strings 2.14.6 and 2.14.7 added
> **after** it, and 2.14.6 added its two rows at its close. The two rows above
> are 2.14.7's, and with them **§11.3 covers every user-facing string this story
> shipped** — which is what acceptance criterion 2's _checked by reading every
> string, not by intent_ asks for, and what Task 2.14.10 would otherwise have
> had to go looking for.
>
> One of them is a new kind. Every row above it judges a sentence that makes a
> claim about data; `No volume to draw. The Price region says why.` makes a claim
> about **where the claim is**. The reading that settles it generalises: _a
> deferral claims nothing in either direction, and the thing to check instead is
> that it does not quietly imply what it is deferring about._ That one does not
> — the series failed, so what the store holds is genuinely unknown, and the
> sentence says only where the account lives.
...
~~the `No shares changed hands anywhere` trigger above~~ — **closed 2026-09-24
by Task 3.9.8; two of the three remain.**
