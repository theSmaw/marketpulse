...
- **Do the loop, not just the launch.** Edit a backend source file and a frontend component in the clone and confirm both reload — the two mechanisms have different baselines (~1.1s process restart against a module replacement) and a fresh clone is where a missing build step shows up as a loop that does not work. Then Ctrl-C and confirm zero survivors in the process group with both ports released
