...
- **Write `docs/adr/0008-*` and add the row to `docs/adr/README.md`.** They are numbered in the order written and never renumbered. Context, decision, rejected alternatives, consequences — the consequences are the part that earns its keep. The decisions this story produces:
  - **How the pair is made legible**, including "nothing changed" if that is what Task 1.8.2 concluded, and why the `warn`-and-above silence makes a quieter default a trap rather than an improvement
  ...
  - **Why Storybook is not in `pnpm dev`**, as a decision rather than an omission
