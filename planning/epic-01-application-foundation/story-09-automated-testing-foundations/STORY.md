...
- **`types: []` in `apps/frontend/tsconfig.json` is load-bearing and test setup is where it gets weakened by accident.** A test file reaching for `process.env` or a Node global will fail to typecheck there, and the correct fix is not to add `"node"` to that array — it exists specifically so server-side APIs do not typecheck in browser code
