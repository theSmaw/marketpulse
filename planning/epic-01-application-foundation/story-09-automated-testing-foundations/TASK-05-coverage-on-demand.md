...
- **Check the provider's install cost.** A coverage provider is frequently a separate package (v8 versus Istanbul, or the runner's own), and the two differ in accuracy and in what they do with transformed sources. Under `nodenext` with a transform step in the middle, source maps are the thing that goes wrong: confirm the report points at `.ts` lines and not at generated output, on a real file, before recording the number
