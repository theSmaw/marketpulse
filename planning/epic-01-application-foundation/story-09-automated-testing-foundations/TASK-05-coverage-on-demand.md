...
- **Decide whether coverage fans out or runs once — and note that Task 1.9.2 tilted this without settling it.** Only `test` and `dev` use `pnpm -r` today, and everything else runs its tool once from the root because the reference graph or the project service already covers the workspace in one pass. Three packages with possibly two environments is the case where a fan-out may genuinely be right — and a fan-out produces three reports that then have to be merged or read separately. Say which, and say what a reader does with the output. **The new constraint: there is no root Vitest config**, deliberately, because root `test` is the fan-out and a root `projects` list would be a second entry point meaning "run the tests". So "runs once from the root" is not a flag away — it means **creating the root config 1.9.2 declined**, and the frontend's config is a `mergeConfig` over `vite.config.ts` that a root `projects` entry would have to point at rather than replace. That is a reversal of a recorded decision, which is allowed but has to be argued here rather than arrived at by following the coverage tool's template
- **Exclude the test files themselves from the denominator, and check what `dist/` does to the numerator.** `*.test.ts` files sit under `src/` beside their subjects, so they are inside every `include` the provider will see. Related and worth one check rather than an assumption: Task 1.9.2 found Vitest 4's `defaultExclude` is `['**/node_modules/**', '**/.git/**']` and nothing more, which is why each package scopes `test.include` to `src`; confirm the coverage provider's own `include`/`exclude` defaults are scoped the same way, because `packages/shared` and `apps/backend` both emit a compiled copy of every test into `dist/` — **8 files for 2 test files in shared and 12 for 3 in the backend**, counting the `.js`, the `.d.ts` and both maps — and a provider that instruments those reports coverage of generated output as if it were source
...
- **Record the first numbers, and record what they exclude.** Take the figure per package. Then say plainly what is _not_ in it: the backend's whole process half — signals, exit codes, the shutdown ceiling, the second-signal path, `EADDRINUSE` and both crash handlers — which injection cannot reach at all, and which Task 1.9.3 recorded as out of scope with **Story 1.10 as its owner**, so this is a known hole with a name on it rather than an oversight; `scripts/*.mjs`, which no test imports; and `apps/backend/scripts/dev.sh`, which is read by no tool in the workspace at all. A coverage percentage that silently omits the parts with no runner is exactly the "green tick that means nothing" this story exists to remove
...
- Test files are excluded from the denominator, and no `dist/` copy of a test appears in the report
