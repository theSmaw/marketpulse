...
- **Re-run all six acceptance criteria and re-take every figure rather than citing one.** The record across eleven stories is that the half that is cheap to check has never been wrong and the half that cannot be checked has been wrong nearly every time it is read
...
- **Run the duplicated-sentence sweep with a `grep`, and expect this story to have created work for it.** Two stories have found this class of drift: Task 1.9.7 found a warning standing in **thirteen** places rather than the three recorded, and Task 1.10.8 found **eight** copies of a claim false since Task 1.10.5 and amended only in the copy being edited. **This story's own STORY.md added a tenth copy of the "Conventions from Story 1.1" block**, so the hash comparison should now find ten identical rather than nine — verify that rather than assuming it, since a block copied by hand is exactly how the count starts drifting
