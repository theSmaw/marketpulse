...
- **A page that loads is not a page that is still talking, and on this check that distinction is the whole point (2026-09-04).** Task 1.12.3's poll **stops entirely while the tab is hidden**, so if this runner's browser reports `document.visibilityState` as `hidden`, the page loads, makes its first request and then goes silent — and a check written to observe the _second_ poll would report the deployed environment as broken when nothing is. Task 1.13.1 owes the reading for the chosen tool. The safe shape either way is to assert on the **first** poll's outcome, which happens on mount regardless, and to treat any wait for a subsequent one as conditional on that reading. Note the failure this check exists for is visible on the first poll: a wrong `CORS_ORIGIN` and a missing `VITE_API_BASE_URL` both fail the very first request
...
- A browser-driven check runs against the deployed environment after a deploy, polling rather than checking once, and what it asserts does not depend on a poll the runner's tab may never make
