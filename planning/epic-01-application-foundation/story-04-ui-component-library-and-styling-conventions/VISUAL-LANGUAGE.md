...
- **The chrome's status strip is three cells spread edge to edge** — added 2026-09-16. Each fact gets a column with its micro-label on the top line and its value beneath, so the three labels form a row and the three values form a row, and a detail sentence is ranged with the thing that owns it rather than wherever its own component chose. It was three right-aligned runs of text separated by 40 px until then: nothing said where a detail sentence belonged, so the strip was three lines tall with a ragged right half and an empty left one. **The separator is distance rather than a hairline, and that is where this idiom and the metric strip's part company** — the rule above is right for figures standing next to each other, and a band 1,392 px wide with three readouts on it is the other case: a hairline hugging the left of a cell with 400 px of air after it marks nothing. A divided version shipped for half a day first, anchored right, and left a void where the cells were not
