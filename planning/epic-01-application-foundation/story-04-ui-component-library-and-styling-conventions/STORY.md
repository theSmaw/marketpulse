...
~~One thing that is true today and will not be forever: until Story 1.9 lands, **`pnpm test` passes because there are no tests** — all three `test` scripts are `echo` placeholders that exit 0, and they are now the only placeholders left.~~ **No longer true as of Story 1.9 (2026-09-03)** — `pnpm test` runs ~~**103 real tests** across 13 files (7 in `packages/shared`, 49 in `apps/backend`, 47 in `apps/frontend`)~~ ~~**189 real tests across 19 files (37 in `packages/shared`, 49 in `apps/backend`, 103 in `apps/frontend`) as of Task 1.12.8 (2026-09-04)**~~ ~~**229 real tests across 22 files (37 in `packages/shared`, 89 in `apps/backend`, 103 in `apps/frontend`) as of Task 2.1.8 (2026-09-05)**~~ **287 real tests across 25 files (55 in `packages/shared`, 129 in `apps/backend`, 103 in `apps/frontend`) as of Task 2.3.8 (2026-09-06)** — Story 1.12 took shared from 7 to 37 and the frontend from 47 to 103, **Story 2.1 took the backend from 49 to 89, Story 2.2 to 106 and Story 2.3 to 129, and Story 2.3 took shared from 37 to 55**, and there is no `echo` placeholder anywhere in this workspace. A green tick now means those ~~103~~ ~~189~~ ~~229~~ **287** tests passed, and specifically **not** coverage: `pnpm coverage` is the separate command that measures that, and ~~the backend's process half is unreachable by any runner here~~ — **that stopped being true in Task 1.10.5**, which added `pnpm test:process`: ~~ten~~ **fourteen** tests spawning a real server (Task 2.1.4 added four for the connection pool's drain), and a seventh step in the chain. The coverage figure did not move, because a spawned child is invisible to the runner's instrumentation. The companion note about both apps' `dev` scripts being placeholders stopped being true earlier still — Stories 1.2 and 1.3 made all three real.
