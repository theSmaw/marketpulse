...
> **One of the selection constraints below has since reversed, and nothing in this record changes as a result.** This task ran on 2026-08-30 against a story that named a **dark** theme as primary. On **2026-08-31** that was reversed to a light theme — see `VISUAL-LANGUAGE.md` and STORY.md's _Visual direction_. Every mention of "dark" from here on is the constraint as it stood on the day, kept rather than rewritten, per `docs/adr/README.md`.
>
> The reversal reaches none of the measurements or the conclusion, and it is worth saying why rather than asserting it: **module counts and bundle weights are theme-independent**, and the spike's dense numeric row exercised the same components against a dark ground that it would against a light one. The only line the reversal touches at all is Mantine's rejection, which credits its dark-theme support in passing — that credit was never load-bearing, since Mantine lost on shipping 231 kB of untree-shaken CSS.
