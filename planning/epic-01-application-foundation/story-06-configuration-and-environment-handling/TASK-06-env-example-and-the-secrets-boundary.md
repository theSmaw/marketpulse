...
- **Decide whether the check is a `pnpm verify` step, because that is what makes it real.** A script that has to be remembered is not a staleness answer. `scripts/check-stories.mjs` is the precedent and the shape to copy — plain JavaScript, ESLint and Prettier both cover it, a `globals.node` block already exists for `scripts/**/*.mjs`, and it earns a sixth step chained into `verify`. Two things come with that choice and both are cheap to forget: `verify`'s step list is written down in `CLAUDE.md`, `README.md` and every Epic 1 STORY.md's conventions block, and a check that has never been seen to fail is a check that has never been tested — move a line out of `.env.example` and confirm it exits non-zero naming the key, the way `stories` was proved
