...
- **Write `.env.example` from the schemas, not from memory.** Every variable, with a description, whether it is required, its default if it has one, and a safe placeholder value. If Task 1.6.1's schema can generate or check the list, do that; if it cannot, the staleness question is real and the answer is a sentence in the file saying the schema is the source of truth
