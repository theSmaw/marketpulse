...
- **If there is an environment concept, name the variable deliberately.** `NODE_ENV` is understood by tooling (bundlers, some libraries) and carries meanings the application does not control; a separate `APP_ENV` is the alternative and its cost is a second thing to set. Whichever wins, it goes through Task 1.6.2's `readEnum` — Task 1.6.1 closed the validation decision as **no schema library**, so "the schema" throughout this story means a declared set of readers — and an unknown value fails at startup rather than falling through to a default
