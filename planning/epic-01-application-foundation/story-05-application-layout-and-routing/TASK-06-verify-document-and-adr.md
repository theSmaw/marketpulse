...
- **Re-measure the artefact and the timings.** Modules, JS, CSS and file count against the story's 193 / 300.09 kB / 7.21 kB / 3 baseline, against Task 1.5.1's measured router cost of 253 / 337.82 kB, and against **Task 1.5.2's shipped 261 / 340.10 kB / 8.62 kB / 3** — three figures rather than two, which is what separates what the router cost, what the routes cost and what the chrome and regions cost; `pnpm verify` against 10.5s from a clean tree; HMR against the two figures Task 1.4.6 established — CSS-only 24–130 ms, component 177–280 ms warm — with the same method (`performance.timeOrigin` unchanged proves it was HMR and not a reload) and the same caveat (a hidden tab throttles React's scheduler, so measure in the foreground or state the bound). Story 1.10 inherits the `verify` number
- ~~**Report what the React Compiler rules finally said.**~~ **They said it in Task 1.5.1, before this task's turn came.** `react-hooks/set-state-in-effect` failed `lint` at **error** on a spike route mirroring a URL parameter into state; the router itself provoked nothing, and the same route without the effect lints clean. So the standing question is answered and this task inherits a narrower one: **report what the rules said across Tasks 1.5.2 to 1.5.5** — whether anything fired in code that shipped rather than in a spike that was thrown away, and whether `--max-warnings 0` cost anything real. **Task 1.5.2 has already answered its share: nothing, from five route modules and the shell carrying zero hooks and zero state between them.** Silence across four tasks of shipping code is itself the finding, and it means the 17 rules taken wholesale in Task 1.3.2 are still cheap — but note what that silence is evidence _of_: this story has had almost nothing to hold state about, and Epic 2 is where that stops being true
- **Write the conventions where they will be read.** `CLAUDE.md` gets the operational half — the file tree with the routes and the chrome in it, where route modules live and why, the workshop's router decorator, the `App.tsx` exemption boundary this story settled, the route-path table and what it does and does not close, and the re-measured figures. Tasks 1.5.1 and 1.5.2 already wrote most of that there — the `react-router-dom` trap, the `react >=19.2.7` peer margin, the no-plugin rule, the `src/routes/` entries in the file map, the path table, the `cx()`/TS2375 door and the shipped artefact figures. **Check and correct them rather than restating them**, and note the current-state paragraph will be wrong the moment the chrome lands. `README.md` gets whatever a human needs to run and look at it. Neither gets the reasoning; that is the ADR's
