...
- Relative imports carry `.js` extensions from `.ts` files (`./App.js` importing `./App.tsx`). `nodenext` resolution requires the emitted filename, and dropping it fails `tsc` with TS2835. **Whether Vite tolerates it is still unverified**: Task 1.3.1's entry file had a single bare package import, so what it proved is that the workspace `exports` map resolves through the bundler — not that a rewritten relative extension does. Task 1.3.2 is the first task to ask
