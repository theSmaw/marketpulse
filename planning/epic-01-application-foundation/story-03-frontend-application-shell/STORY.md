...
- **The install-script policy did _not_ fire, and the prediction was wrong.** Task 1.1.1 set `allowBuilds` in `pnpm-workspace.yaml` as an allowlist, and an un-allowlisted dependency with an install script is a **hard install failure (exit 1)**, not a warning. esbuild was the predicted first trip. But Vite 8 uses **Rolldown**, which ships as prebuilt per-platform binaries, and Task 1.3.1 found **zero** install scripts across the entire tree — as did Task 1.3.2 (React) and Task 1.3.3 (`@vitejs/plugin-react`). Three tasks, three sweeps, nothing. `allowBuilds` remains empty and remains untested. The rule is unchanged for whenever something does bring one — allowlist that package by name, never disable the check
