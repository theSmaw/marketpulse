...
- **`pnpm verify` now includes two bundlers, so CI's runtime and failure surface both grew.** Task 1.4.5 added the second: root `build` also runs `storybook build`, which is the slower half — measured at ~8.3s for the whole chain against ~7.6s before, most of the difference being Storybook's own bundle. `storybook-static/` is a candidate CI artefact and this story owns whether it is published.
