...
- **Re-take the runner figures against a known spread, not against a single reading.** Task 1.10.2 measured the same commit at **31,075 ms** and **21,989 ms** on two runners, cold install 5,606 and 5,857 ms, and a warm in-job re-run at 18,953 ms where only `build` moved. So three readings are the minimum for any claim about a total here, and the per-step split is still the durable half. A single slower run after Tasks 1.10.3–1.10.5 is not evidence of anything
