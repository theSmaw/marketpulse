...
- **Add the new gap this story creates.** `.github/workflows/*.yml` is YAML: ESLint does not read it, Prettier's YAML parser will format it if it is not ignored (check which, and say), and nothing validates the schema. The pipeline's own definition is the one file whose breakage is invisible until a run fails — or worse, until a run silently stops triggering. Record it with the others rather than letting it arrive unlisted
...
- The fifth — the workflow file itself — is added to the list, with Prettier's actual treatment of it determined rather than assumed
