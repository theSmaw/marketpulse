...
- **Know what a second fan-out costs on this runner before choosing where it goes.** `pnpm coverage` is 2.92 s locally, and Task 1.10.2 measured the runner at **2.2–3.5× the laptop per step** with a chain of 21,989–31,075 ms and a **9 s spread between two runners on the same commit**. So coverage appended as a step to the `verify` job lengthens the answer everyone waits for by roughly the amount two runners already differ by; in its own job it costs wall-clock nothing. That is an argument for a parallel job independent of the `continue-on-error` question
