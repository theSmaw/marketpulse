...
- ~~**The backend already logs, at Fastify's defaults, and this story replaces that rather than introducing it.**~~ **Two thirds done.** Task 1.7.1 added the configurable level and the rendering; Task 1.7.2 replaced the inherited request record with a chosen one and got the correlation id out of the process as an `x-request-id` header. What is still missing against the criteria below is **any error shape at all** — Tasks 1.7.3 and 1.7.4 — and the crash handlers
