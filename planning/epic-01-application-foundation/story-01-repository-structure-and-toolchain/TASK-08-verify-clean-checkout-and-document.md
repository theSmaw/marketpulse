...
Re-checked at Task 1.1.5 before installing: typescript-eslint 8.68.0 is still `latest` and its range is still `>=4.8.4 <6.1.0`. The gate has not opened. Record the check as *repeatable*, not as a settled fact with a date attached.
