...
Validation is a format check and nothing more — `/^[A-Z]{1,5}(\.[A-Z])?$/`, matching Alpaca's US-equity shape including share classes (`BRK.B`). Whether a ticker is actually _listed_ is a question for the security universe in Epic 2, not for a string predicate.
...
- **Relative imports carry `.js` extensions from `.ts` files** — `./ticker.js`, not `./ticker`. `nodenext` resolution wants the _emitted_ filename. `src/index.ts` carries a comment saying so.
