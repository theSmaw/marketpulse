...
**The deployed database is migrated from a GitHub Actions runner, connecting as
`marketpulse-github-deploy` — this repository's own federated identity — and not
as the backend.** That is forced rather than chosen. Task 2.1.6 measured that a
service principal cannot mint a token for another principal's Postgres role
(_"Service principals cannot generate AAD_AUTH_TOKENTYPE_APP_USER tokens for
role"_), so CI could not connect as `marketpulse-backend` even if that were
wanted. Least privilege therefore came free: the runtime role holds no DDL rights
at all, and the migration role holds no more than `CREATE` on `public`.

| Piece                     | Value                                                                                                                                              |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| Migration identity        | App registration **`marketpulse-github-deploy`** — the same one `deploy.yml` signs in with                                                         |
| — service principal oid   | `f8b785a8-f6e1-4ca9-a71a-a906e5356d6a`                                                                                                             |
| — application (client) id | `1bb765eb-fff3-4aed-80f2-90796c2fbcfb`                                                                                                             |
| Postgres role             | `marketpulse-github-deploy`                                                                                                                        |
| Token resource            | `https://ossrdbms-aad.database.windows.net`, minted on the runner by `az account get-access-token` and passed as `DATABASE_PASSWORD`               |
| Role creation             | `select * from pgaadauth_create_principal_with_oid('marketpulse-github-deploy', 'f8b785a8-f6e1-4ca9-a71a-a906e5356d6a', 'service', false, false);` |
| — run where               | on the **`postgres`** database, connected as the Entra administrator                                                                               |

**The grants, run on the `marketpulse` database as the Entra administrator.**
These are the whole of the migration role's authority and they are recorded here
because they exist nowhere else:

```sql
grant connect on database marketpulse to "marketpulse-github-deploy";
grant usage, create on schema public to "marketpulse-github-deploy";

-- Tables the migration role creates are owned by it, so the runtime role needs
-- to be granted access to them. Default privileges do it once rather than per
-- migration — which matters, because "and grant the backend access" is exactly
-- the line somebody forgets in the migration that adds the table.
alter default privileges for role "marketpulse-github-deploy" in schema public
  grant select, insert, update, delete on tables to "marketpulse-backend";
alter default privileges for role "marketpulse-github-deploy" in schema public
  grant usage, select on sequences to "marketpulse-backend";
```

The sequence grant is not needed by anything today — a `generated always as
identity` column carries its permission through the table — and is there for the
first explicit sequence, because its absence presents as a confusing runtime
permission error rather than as a missing grant.

**Read back after the first deployed migration, which is the point of writing it
down:** all four tables in `public` are owned by `marketpulse-github-deploy`;
`marketpulse-backend` has `SELECT`, `INSERT`, `UPDATE` and `DELETE` on
`securities` and `has_schema_privilege(…, 'public', 'CREATE')` is **false**.

**Two traps in re-running this bootstrap, both hit while doing it.**

**`pgaadauth_*` exists only in the `postgres` maintenance database.** Called from
`marketpulse` it is `function pgaadauth_create_principal(unknown, boolean,
boolean) does not exist`, SQLSTATE `42883`, and a `pg_proc` sweep of that database
returns nothing at all. Roles are cluster-wide, so it does not matter that the
role is created from a different database than the one it will use — but the
connection has to be to `postgres`. The row above says so now; Task 2.1.5's did
too, and it is the kind of line that is easy to read past.

**`pgaadauth_create_principal_with_oid` was used rather than the name form.** The
name form resolves the principal by looking it up in Entra; the oid form states
it. For a service principal that is the difference between depending on a
directory lookup and depending on a value that is also in `deploy.yml`.

### What the migration role can do to a table it owns — Task 2.3.7's question, answered

**Task 2.3.7 loads 101 rows into `securities` from the same runner and as the
same role, and the open question was whether the grants above cover a loader
rather than a migrator. They already did, and nothing was added — so this section
gains no `grant` statement, which is the outcome worth recording.**

The mechanism is ownership rather than a grant: `marketpulse-github-deploy` owns
all four tables in `public` (read back from `pg_tables`), and ownership carries
DML. Confirmed twice on 2026-09-05, weakly and then strongly.

`has_table_privilege('marketpulse-github-deploy', 'securities', …)` is **true**
for `SELECT`, `INSERT`, `UPDATE` and `DELETE`. But a privilege function is a
claim about a privilege rather than about a statement, so **every verb the loader
actually issues was executed against the deployed table and rolled back**, under:

```sql
begin;
set role "marketpulse-github-deploy";
-- the upsert, the untrack that Task 2.3.6 added, and the sweep's select
insert into securities (...) values (...) on conflict (symbol) do update set ...;
update securities set status = 'untracked' where symbol = ...;
select count(*) from securities;
rollback;
```

All three succeeded as that role. Two things fell out of it worth keeping. The
`set role` route works from the Entra administrator's own session, which is the
cheap way to test this role's authority **without** a runner and without a
federated credential — the alternative Task 2.2.7 had to build a throwaway branch
for. And the rolled-back inserts still **consumed identity values** (ids 1 and 2
are absent from the loaded table, whose ids run 3–103), which is the recorded
identity-sequence behaviour arriving on the managed server.

### A GitHub-hosted runner can reach this server, measured rather than reasoned about

**Task 2.2.7's required measurement, taken on a throwaway branch with a temporary
federated credential, both since deleted.** The `AllowAllAzureServicesAndResources`
firewall rule admits every Azure-internal address, and a GitHub-hosted runner is
an Azure virtual machine — its egress address on the run taken was
**`172.174.110.129`**, which the rule admits. So no firewall rule per run is
needed, and nothing writes to platform state from CI.

Measured on that run: **connect in 142 ms** including TLS `verify-full` and the
token (already minted by `az` in a prior step), `current_user` reading
`marketpulse-github-deploy`, `has_schema_privilege('public', 'CREATE')` **true**,
`public` holding **0 tables** beforehand, and `pnpm migrate` applying both
migrations in **1.181 s** wall. Compare the deployed replica's first connection at
~1,023 ms, of which 866 ms is the sidecar token mint the runner does not pay.

**The probe branch needed its own federated credential**, because the two that
exist are both scoped to `refs/heads/main` — which is worth knowing before
planning any CI work that touches Azure from a branch. It was added, used and
deleted; the credential list is back to the two `main` subjects.
...
**There are THREE pins now and exactly one pair of them is compared.** Task 2.2.5 added a `postgres:18` service to the `database` job in `verify.yml` and, rather than leaving a second silent pin, made a step read `LOCAL_DATABASE_VERSION` out of `scripts/local-database.mjs` and compare it against what the service reports — so local-versus-CI drift is a red job naming both numbers. **The deployed pin is still compared by nothing**, and it is the one whose drift has the "works locally, wrong in production" symptom this pin exists for. A check for it stays refused for the reason it always was: it needs Azure credentials `pnpm verify` deliberately does not have, so building one forks the definition of "verified". It is a stated invariant of the third kind, and it is in the gap lists in `README.md` and `CLAUDE.md` for that reason.

**So it is taken by hand, and Task 2.2.7 was the cheapest place it will ever be taken** — that task connects to both. Read on **2026-09-05**: the local container reports **PostgreSQL 18.6 (Debian 18.6-1.pgdg13+2)**, and `select version()` on the managed server reports **PostgreSQL 18.6 on x86_64-pc-linux-gnu, compiled by gcc (GCC) 13.2.0, 64-bit** — from a laptop as the Entra administrator and again from the runner as `marketpulse-github-deploy`. The number is recorded rather than the fact that it matched, because "it matched" is not re-checkable and **18.6** is.
...
**The firewall has exactly the two rules Task 2.1.1 specified** — `AllowAllAzureServicesAndResources` (`0.0.0.0`–`0.0.0.0`, i.e. every Azure tenant's compute) and `developer-laptop` (a single IPv4 address). Nothing else is admitted. **What is deliberately not enabled** is any broader developer range: one address, changed by `firewall-rule update` when it moves. **It has moved twice on 2026-09-05** — `122.11.246.19` → `58.182.90.91` at Task 2.2.7, and then **back to `122.11.246.19` at Task 2.3.7**, which is worth more than the addresses: the hazard is not that the IP drifts one way, it is that it changes on almost every task that needs an operator connection, and the symptom is a laptop timing out against a server the deployed backend reaches perfectly. **Do not quote any of these addresses as current — read `api.ipify.org` and compare.** The command is `az postgres flexible-server firewall-rule update -g rg-marketpulse-dev -s psql-marketpulse-dev -n developer-laptop --start-ip-address <ip> --end-ip-address <ip>`, and note `-s` is the server and `-n` is the rule, which is the opposite of what the plausible guess (`-n` for the server) does — that form is rejected outright.
...
| Database server name       | **`psql-marketpulse-dev`**, FQDN **`psql-marketpulse-dev.postgres.database.azure.com`**. Database **`marketpulse`** (UTF8 / `en_US.utf8`) — **migrated by Task 2.2.7 and holding `securities`, `kysely_migration`, `kysely_migration_lock` and `migration_checksum`. ~~All empty of rows~~ — since Task 2.3.7 `securities` holds the 101-row tracked universe, all `active`**. Firewall: exactly two rules — `AllowAllAzureServicesAndResources` (`0.0.0.0`) and `developer-laptop` (one address, which moves) |
| Database admin             | **No PostgreSQL admin user exists** — `administratorLogin` reads `null` on the created server. The Entra administrator is `bensmawfield_outlook.com#EXT#@bensmawfieldoutlook.onmicrosoft.com` (`8d92279d-ed7d-4127-9884-ba258857457c`), **held by Postgres truncated to 63 characters**, losing the final `om`. Roles `marketpulse-backend` and — since Task 2.2.7 — `marketpulse-github-deploy` exist via `pgaadauth_create_principal`. **No password anywhere**                                              |
