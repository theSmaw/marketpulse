...
**The firewall has exactly the two rules Task 2.1.1 specified** — `AllowAllAzureServicesAndResources` (`0.0.0.0`–`0.0.0.0`, i.e. every Azure tenant's compute) and `developer-laptop` (a single IPv4 address). Nothing else is admitted. **What is deliberately not enabled** is any broader developer range: one address, changed by `firewall-rule update` when it moves. **It moved on 2026-09-05** — `122.11.246.19` → `58.182.90.91`, the recorded hazard arriving on the first task that needed an operator connection after Task 2.1.6, so do not quote either address as current. The command is `az postgres flexible-server firewall-rule update -g rg-marketpulse-dev -s psql-marketpulse-dev -n developer-laptop --start-ip-address <ip> --end-ip-address <ip>`, and note `-s` is the server and `-n` is the rule, which is the opposite of what the plausible guess (`-n` for the server) does — that form is rejected outright.
