...
- **Point `CORS_ORIGIN` at the deployed frontend's exact origin** — scheme, host and port, matched as a string. This is the value Task 1.11.3 left as **`https://placeholder.invalid`** — explicitly not the `http://localhost:5173` default, and `.invalid` is RFC 2606 reserved so it can never resolve. Closing it here is the reason these two tasks are ordered this way. The backend it points at is live at `https://marketpulse-backend.blackgrass-e682fefb.eastus.azurecontainerapps.io`. **It is the production environment's origin and no other**: Static Web Apps gives every pull request and every branch environment its own hostname, and the allowlist holds one string, so **a preview environment cannot talk to this backend and that is by construction rather than by omission**. Say so where somebody testing from a preview URL will read it, because the symptom they will get is the `TypeError: Failed to fetch` described below, with a healthy 200 in the server log — indistinguishable from the misconfiguration this task exists to prevent
