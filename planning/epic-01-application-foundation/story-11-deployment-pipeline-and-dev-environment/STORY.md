...
One thing, and it is a second artefact rather than a change to either of the two above. `pnpm build` now also emits **`apps/frontend/storybook-static/`** — 50 files and 7.4 MB, none of it part of the product. It is a static site like the frontend and could be deployed the same way, but **Story 1.10 owns whether CI publishes it** and this story should not deploy it by accident: a `dist`-shaped glob or a "copy everything the build produced" step will pick it up. Decide it deliberately, in one place, with 1.10.
