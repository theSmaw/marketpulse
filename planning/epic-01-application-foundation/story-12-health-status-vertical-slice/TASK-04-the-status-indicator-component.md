...
- **The `requestId` display rule is already taken and this component inherits it rather than re-deciding it.** Task 1.12.2 settled it beside the type that carries one: the whole id and never a prefix, only ever as a labelled reference beside a failure the user is already being told about, and never as the message. A status indicator in the chrome is almost certainly **not** that place — it reports a state, not a failure a user is being asked to report — so the likely correct answer here is that it shows no id at all. Say which, rather than leaving it unstated
