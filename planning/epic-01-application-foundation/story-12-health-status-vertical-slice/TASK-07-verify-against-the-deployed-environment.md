...
- **Decide the browser smoke check, because this story is its named reversal trigger.** Task 1.11.7 declined a post-deploy check that drives a real browser, with the gap stated: only a real browser catches a wrong `CORS_ORIGIN` or a missing `VITE_API_BASE_URL`, and `curl` is structurally incapable of it. **This story is the first one capable of shipping that failure.** Take the decision and record it with its reason; a check added by reflex and a gap left unmentioned are both wrong answers
