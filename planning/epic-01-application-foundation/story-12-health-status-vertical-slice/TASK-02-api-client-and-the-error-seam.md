...
- **The body parse is already written — import it rather than writing a second one.** Task 1.12.1 shipped `isHealthResponse(value: unknown)` in `packages/shared/src/health.ts`, beside the interface it checks, precisely so the definition of `degraded` is executable and cannot drift from the shape it validates. A validator written again here is a second definition of the same judgement, and it is the copy that will disagree first. Two properties of it to know before wrapping it: it **accepts** a `status` member this client has not been taught and it **accepts** unknown extra fields, because a newer server is a version skew rather than a broken one — the whole reason `HealthStatus` is a union that can grow. What it rejects is a missing field, a wrong primitive type, and a non-object, the last being the string body a static host returns
...
- Give the request a **timeout and an abort signal**. `AbortSignal.timeout()` is what makes "something is on the port and it is not this server" a distinct outcome from `ECONNREFUSED`: a socket that accepts and never answers hangs `fetch` forever, measured in Task 1.8.4. Without one that socket reaches **no** state at all rather than the wrong one. **Task 1.12.1 settled which side of the line a timeout falls on: `unreachable`, because nothing arrived** — `degraded` is defined structurally, as an answer that came back and was not a health report. So the deadline is the boundary between "answered" and "did not answer", it is the number this module owns, and it is the only reason the hung-socket case is distinguishable from a healthy slow one
