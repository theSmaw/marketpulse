...
- The frontend must reach the backend over HTTP without importing anything server-side. `apps/frontend`'s `types: []` enforces that structurally — `process` does not typecheck there — so resist any convenience that requires weakening it. Note the shape this takes in practice: the browser's `fetch` is available through the `dom` lib, and **there is no environment-variable mechanism in the frontend at all** — no `import.meta.env` usage, no `.env` file, nothing. The backend's URL has to come from somewhere this story chooses
