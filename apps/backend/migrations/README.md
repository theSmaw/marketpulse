...
| A closed set's union and its `check` constraint agree            | `pnpm test:database`, parsing the constraint Postgres **rewrote** — table-driven over all three of `securities`' vocabularies |
| A cross-column invariant a row type cannot express               | `pnpm test:database`, behaviourally — `securities_sector_matches_kind`, refused both ways and accepted once                   |
| A provenance column is `not null` with no default                | `pnpm test:database` — an insert omitting one is refused by name                                                              |
