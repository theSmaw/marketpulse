...
// Seventeen bodies `GET /market-data/bars` answers with, recorded from the real
...
//
// **Sixteen of the seventeen came off a real store.** The exception is
// `holidayWeek`, added by Task 2.13.8, whose window is in the **future**; its
// entry argues in full why a body generated by this product's own server against
// the checked-in calendar is admissible here and a hand-written one is not.
//
...
// of the seventeen below are deliberately in exactly that state, and they are labelled
...
/**
 * The three transport outcomes these seventeen bodies can be.
 *
...
export const BAR_SERIES_FIXTURES = {
  ...
  /**
   * **The five sessions of Thanksgiving week 2026, complete** — acceptance
   * criterion 3's window, and the one body here that no store could answer.
   * → `loaded`.
   *
   * Added by Task 2.13.8. `2026-11-23 … 2026-11-30` is in the **future**: there
   * are no bars for it anywhere, and there is no `curl` that can ask for it.
   * That is also why the criterion cannot be proved *through the control* —
   * §4(a) settled the address as `?sessions=N`, a count the server resolves
   * against the market date it is answering on, so no press of any cell and no
   * hand-typed URL reaches November from September.
   *
   * **So the body was generated by this product's own server against the real
   * calendar**, which is the distinction that makes it admissible here rather
   * than a hand-written body this module exists to refuse. The shipped route —
   * the same plugin `index.ts` registers, the same `toStoredSeries`, the same
   * response schema — was driven through `app.inject()` with its clock seam
   * pinned to Monday 2026-11-30 21:05Z, over rows generated one per trading
   * minute of `lastMarketSessions(5, "2026-11-30")`:
   *
   * ```js
   * const sessions = lastMarketSessions(5, toMarketDate("2026-11-30"));
   * for (const session of sessions)
   *   for (let m = 0; m < session.minuteBars; m += 1)
   *     rows.push({ observed_at: new Date(session.open.getTime() + m * 60_000), … });
   * ```
   *
   * **Nothing about the window is invented and nothing about the prices is
   * real**, and the split is the point. The instants, the session boundaries,
   * the bar counts and the coverage all come from the checked-in calendar and
   * from the route: 390 bars on `11-23`, `11-24`, `11-25` and `11-30`, **210 on
   * the half day `11-27`**, **nothing at all on Thanksgiving `11-26`**, 1,770
   * in total, and a `requested` range of `2026-11-23T14:30:00Z` to
   * `2026-11-30T21:00:00Z` — which is 09:30 to 16:00 in a week that is on EST
   * rather than EDT. The prices are a seeded random walk, because no assertion
   * anywhere reads one: what this body is for is *where the bars fall on an
   * axis*.
   *
   * **357 KB, which makes it the largest thing in this directory** — ahead of
   * `dense`'s 222 KB — and therefore the head of `CLAUDE.md`'s
   * *must-not-ship* list. Re-measure by name:
   * `grep -o "2026-11-27T18:5[0-9]" apps/frontend/dist/assets/*.js` must find
   * nothing, and that string is distinctive because 18:5x on `11-27` is the
   * last ten minutes of a session that closed at 13:00 ET.
   */
  holidayWeek: {
    status: 200,
    body: HOLIDAY_WEEK,
    outcome: "ok",
    describes:
      "five complete sessions across Thanksgiving week, half day included",
  },
