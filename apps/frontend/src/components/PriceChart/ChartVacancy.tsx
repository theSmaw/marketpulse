 ...
 *
 * **The two headlines are whole literals rather than one with the subject
 * interpolated**, and that is not a style preference: `pnpm invariants` proves
 * the price sentence exists in exactly one source file, and a grep cannot see a
 * string a template assembles. A check that cannot find the thing it guards
 * passes for the wrong reason, which is the failure mode `CLAUDE.md` names —
 * one of the seven invariants had already rotted that way.
 *
      ...
      <div className={styles.content}>
        ...
        {subject === "bars" && (
          <p className={styles.detail}>
            We asked for {formatMarketRange(requested)} and hold nothing in it.
            A window reaching into the current session is usually this: stored
            history is caught up overnight.
          </p>
        )}
      </div>
    </div>
