...
something in it. **Since Stories 2.11 and 2.12 that is two routes and the second
one draws**: `/securities/:symbol` deep-loads cold and renders a real price
chart. Everything else below Epic 2 is still a shell on purpose.

**Since Epic 2 closed (2026-09-15) that second route says where its numbers came
from.** A source note at the foot of the Explorer states the adjustment, when the
bars were retrieved, and that sector and industry are curated rather than market
data — and it names the series' own feed when the chrome cannot. A short answer
says how far it reaches, in full instants. An empty answer says which kind of
empty it is: _no history stored for this security yet_ and _no bars stored for
this window_ are different sentences, because they imply different next actions.
...
| `/securities`         | Security Explorer                      | **the tracked universe** — 518 real securities from the database with a real last close each, grouped into twelve bands with a rail that jumps between them and a control that shuts them all, plus a **search field** that matches as you type and opens a security                                                                                                                                                                                      |
| `/securities/:symbol` | one security                           | **a price chart and a volume chart on one shared axis** — that security's real bars over a window the reader chooses (`1D 5D 1M 3M 1Y`, held in the address), with a crosshair and a keyboard reading, the exact figures beneath each, an honest sentence for every empty, partial and failed answer, and a source note saying where the numbers came from; plus §8.3's other four regions, each naming the epic that fills it (Epic 3, Epics 5, 6, 8, 9) |
