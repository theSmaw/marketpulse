...
files still agree with the configuration table, all **507** fast tests passed, and
...
and a test suite of **507** fast tests plus a 14-test process suite, with
...
| `pnpm test`         | Every package's tests — 507 across the workspace — see below          |
...
`apps/backend` 146 across 10, and `apps/frontend` 163 across 18 — **507 in
...
**It is a separate command because it is a separate cost.** `pnpm test` is 507
...
It is the same 507 tests with `--coverage` added, fanning out through
