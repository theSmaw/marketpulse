...
**Base UI's real cost landed here, and it is larger than the story's headline number.** The application bundle went from 196.36 kB to **293.06 kB** when the popover arrived — about 97 kB — against Task 1.4.1's measurement of Radix being lighter by 37 kB for one primitive. That reversal was taken knowingly and on a constraint no spike could produce; the number is recorded so nobody re-derives it or reads the growth as a regression. The stylesheet went 6.22 kB → 7.05 kB, and the artefact is still three files.
...
**The React Compiler rule set has still never fired**, five components in. Nothing in the tree is stateful and there are no hooks anywhere, so the 17 rules adopted wholesale in Task 1.3.2 remain unexercised — Task 1.4.5 predicted it would be the first real test and was wrong, because CSS Modules compute nothing during render. Story 1.5 or Epic 2 will be the first, and it may not be welcome then.
