...
**Since 2026-09-12 that line also says which way the window went, without depending on colour.** A dashed rule sits at the price the window opened at — the **first bar the store holds, and that bar's `open`**, which is the same number the percentage above the plot is computed from — and the area between it and the line is washed green, red or neutral. The geometry is the channel and the tint repeats it: the two washes differ by **1.009:1 under `grayscale(1)`**, so cover the hue and the chart still reads. Verified as a rendering rather than as a claim — four windows under greyscale and under a deuteranopia matrix, as stories in the workshop. The high–low extent band was drawn, measured and **declined** at `1m`; it returns with Story 2.13's `1d`.
