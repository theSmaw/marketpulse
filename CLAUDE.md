...
**Both are skeletons in scope, not in status.** The backend has no market data, no database, no domain logic, no configuration module and no CORS. The frontend has no routing (Story 1.5), no state management, and does not call the backend (Story 1.12). Its styling system is **decided, piped, designed at the token level and now carrying the product's own vocabulary**: Story 1.4 picked CSS Modules plus CSS custom properties for styling and **Base UI** (`@base-ui/react`) for behaviour (Task 1.4.1 — Radix was the original choice and was reversed on 2026-08-31, on the intention to swap in a Base-UI-based shared component library later rather than on the measurements, which favour Radix by ~46 kB), Task 1.4.2 got the first stylesheet into the build with no dependency at all, **Task 1.4.3 defined the tokens and the theme**, and **Task 1.4.4 has now layered the market semantics over them** — price direction, a four-band anomaly ramp, and feed status. What is still missing is components: Base UI is not installed until Task 1.4.5, which is also where the rule that every Base UI usage sits behind our own thin wrapper is established, and where the pairing of every semantic colour with a second channel stops being a convention and gets a component behind it. Neither has tests (Story 1.9). Stories 1.4–1.12 can all proceed from here.
