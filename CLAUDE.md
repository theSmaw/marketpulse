...
**And the chrome says whether the data behind it is arriving.** Since
2026-09-19 the status bar's market-feed cell carries **two** facts — the venue
and the connection — read from a live WebSocket rather than hard-coded, on all
five routes. `LIVE`, `STALE`, `DISCONNECTED` and `REPLAYING` are shipped words
with one home, and a deployment with no provider renders **no connection word at
all**, which is the grid's `—` rather than a claim that something broke. Close
...
bars sit, joined through `mergeSeriesProvenance`. ~~**Nothing on any screen
reads it yet**, and no deployed store holds two tapes until Story 3.8 writes
one~~ — **both halves ended on 2026-09-23, see the next paragraph.** The
migration was **two catalogue writes** and its deployed step took **1.251 s**
inside `deploy.yml`'s `timeout 120`. [`TAPE.md`](planning/epic-03-live-market-data/story-07-the-tape-on-the-bar/TAPE.md)
is the subject document and [ADR 0034](docs/adr/0034-the-tape-on-the-bar.md) the
decision.

**And since 2026-09-23 the deployed backend WRITES the session down, which is
the first thing in this epic a reader meets by reloading.** Open a security
mid-session, reload, and the chart still reaches the minute it reached before —
from `market_bars` rather than from a socket that has been running since the
page opened. Every complete minute bar the live stream delivers is stored with
its tape, for six and a half hours a day, by a writer that **never throws**
(it is called from the socket's own callback, so every refusal is caught per
security, counted and logged). **Both tapes are kept** — a minute may hold one
`iex` row and one `sip` row, because a stored bar is a record of an observation
rather than a cache of the best number — and the served read picks **one**,
preferring `sip`, because `toBarSeries` refuses bars that are not strictly
ascending and the alternative was a 500 on a page load.

The consequence is the part worth knowing before designing against it: **a
served window's `sources` describe the rows the ANSWER contains, not every tape
the store holds for it.** So the two-feed source note — `All US exchanges` over
`IEX` with the one-venue sentence, which this product shipped the code for two
epics ago and had never been able to show — is a **mid-session** state: any
window whose last session is today, while the market is open. After that
night's backfill the same window names one `sip` source, because every
regular-session minute then has a consolidated version. Measured against
production on 2026-09-23 after the backfill: `NVDA`, `1m`, one session, 390
bars, **one** source. The stretches that survive the night are **extended
...
**And since 2026-09-24 the product is honest when its feed is not — Story
3.10, the epic's last feature story.** `live | stale | disconnected` has one
home, the status bar, with a sentence and the instant the data was good to;
every other surface on the security page stays quiet **by decision**, each
taken with a measurement behind it. Killing the feed leaves `main`'s text
**byte-identical** and raises no page error in any of nine produced states. A
quiet socket at 02:00 is `LIVE`, a quiet security is an **age rather than a
verdict** (no threshold, the row dated `Live price from 12:07` when behind),
and a dropout **fills its own gap** when the socket returns. The cell announces
a degradation and nothing else. `PRODUCT_SPEC.md` §36 carries a dated amendment
saying how the shipped sentence differs from its example and why.
...
- **A listening pass with a real screen reader — ONE item, SIX entries since 2026-09-24.** The sixth is the first that is not the chart's: the market-feed cell announces a **degradation** and nothing else (Task 3.10.6), and since Task 3.10.5 that cell is the only surface on a security page that says the feed stopped — so whether a `role="status"` region empty on mount and later filled is spoken **at all** is a listener's whole notice, and it is unanswerable from a DOM. The fifth is the first that does not need a key pressed at all: since the chart extends on its own (Task 3.9.2), a **revision of the bar under the crosshair** rewrites the reading's polite region unprompted — measured, and rare at **0.1%** of bars, and arguably the one case where speaking is right because the number under the reader's cursor has just changed. What nobody can say is whether an unprompted polite update **queues behind a sentence in progress or replaces it**, which is the same unanswerable as the entry below arriving by a new route (Task 3.9.6). The spoken bar sentence is 25
...
- ~~**The security page spends 50–66 ms of main thread on a cold load**~~ —
  **owned since 2026-09-15 rather than open.** The breach is real and unchanged:
  ...
  `SEARCH-AND-SELECTION.md` §10 holds all three datings. **Re-taken
  2026-09-22 by Task 3.6.5 with the feed running: 50–56 ms on seven loads in
  ten** after a fixed table layout from 1024 up, and the trigger evaluated in
  writing — it did not fire as worded, but a **routine** per-tick breach the
  live feed had introduced (46–49 ms of script a minute, 40 ms every 30 s from
  the health poll re-rendering the route) was found and repaired there with two
...
- ~~**Two shipped sentences are correct today and become false the first time an
  ...
  **expires that night**, owned by Task 3.8.10. The history below is left
  ...
  until Story 3.8 writes one~~ — **Story 3.8 wrote one on 2026-09-23, and the
  qualification that replaces it is about WHEN rather than whether**: the
...
- **A rehearsal proves only what its conditions can produce, and the part that handles traffic needs traffic.** Found 2026-09-25 by an overnight run: both browser-side instruments in `scripts/` drained their page buffer by **rebinding the global to a fresh array** while the page's wrapper kept pushing into the array it had closed over — so **every drain after the first returned nothing**, and the instrument reported a silent socket on a perfectly healthy connection. `session-sitting.mjs` carried it through a rehearsal that Task 3.11.8 recorded as clean, because with the market shut there are no frames after the first drain and **one drain and all drains look identical**. The repair is `splice` in place; the rule is that a rehearsal against a quiet system certifies the wiring and **not** the loop, and the findings section should say which half it proved. **The defect is not _rebinding a buffer_** — the two shipped sites that do (`replay-bar-source.ts`, `backfill.ts`) are correct, because every producer reads the same `let` by name and a rebind is visible to all of them. It is **rebinding one of two bindings to the same array**: the page wrapper held `out` as a `const` in its own scope while the drain replaced `globalThis.__gap`, so one binding moved and the other kept filling a array nobody read. Swept 2026-09-25 across the frontend, the backend and shared: no shipped code has that shape.
...
**Since 2026-09-11 that file is no longer the origin of the language** (ADR 0026). The `Component library for MarketPulse` design canvas is the source of truth, reached with the `DesignSync` tool, and the chain is **canvas → `VISUAL-LANGUAGE.md` → `tokens.css` → components**.

**The canvas is at `727b5b14-fe78-47c1-9d9c-fb84b6ce5280`** — `https://claude.ai/design/p/727b5b14-fe78-47c1-9d9c-fb84b6ce5280`, and the last segment is the `projectId` every `DesignSync` method takes. **Reach it with `get_project`/`list_files`, NEVER with `list_projects`**: that method filters to **design-system** projects and this one is not, so the canvas comes back **absent rather than listed**, which reads exactly like _the canvas does not exist_. **Two documents have carried that warning since 2026-09-11 and 2026-09-19** — `VISUAL-LANGUAGE.md`, and Story 3.4's own `STORY.md`, which spells out the cause and calls the wrong conclusion _an easy one to reach_ — and **three stories reached it anyway**: 3.7, 3.8 and 3.9 each wrote the absence down as a fault, each escalating it, and 3.8's close asked the owner to investigate a tool that was working. The address is here because the sentence that prevents it was one file further than anybody looked. Where the document and the canvas disagree, the document is wrong; downstream the old rule is unchanged, and a component still may not diverge from the document. There is **one standing exception**: where a canvas value fails a measured accessibility floor, the intent is adopted and the value is not, with the measurement recorded beside the token. It has fired four times — the input boundary, the placeholder ink, a validated tick the canvas drew in the green this product spends on price-up, and the source note's micro ink, measured at 4.26:1 on the page ground on 2026-09-14. Two things from it that are settled and load-bearing: **colour is never the sole encoding of anything** (the price palette differs by 1.04:1 in greyscale, so hue is the entire difference — shape, sign, glyph or word must carry it), and **standing out, like receding, is a job for weight and hierarchy, never for ink outside the contrast floor**. Both have caught real defects.
...
- **A throw inside a React effect is a render error, and nothing above `App` catches one — the page goes blank.** A `WebSocket.send` on a socket in the `CLOSING` state throws, and `market-stream-client.ts` reached one from a subscription effect in the 500 ms between a gateway's close and the retry's dial (found 2026-09-22 by the reconnect spec, three tests at once, on a developer's machine and not on CI). The rule that follows: **a transport asks the socket's own `readyState` before every send**, never a flag it set on `open`, and anything an effect calls on a resource somebody else can close needs the same question asked of the resource rather than of a local.
...
- **The deployed backend WRITES bars, every minute the market is open, since 2026-09-23.** `live-bar-writer.ts` hangs off the live stream in `index.ts`, so `market_bars` has a writer running **inside the deploy window, in the image about to be replaced**, for six and a half hours a day — not only a nightly backfill on a GitHub runner. A migration on that table therefore meets a live writer, and that writer **never throws and never fails loudly**: it is called from the socket's own callback, where an unhandled rejection is a crashed process, so every refusal is caught per security, counted and logged. A write-shape change on this table is two deploys, expand then contract. Anything in this repository written before that date assumed the backfill was the only writer.
- **A full scan of `market_bars` on the deployed tier is minutes, not seconds, and the deploy gives a migration two.** Measured 2026-09-22 (Task 3.7.1, ADR 0034): on PostgreSQL 18 `add column … not null default '…'` and `add constraint … check (…) not valid` are **catalogue writes** — 39 ms and 3 ms on 48.8 million rows — but `validate constraint` read the whole 5 GB heap in 6.2 s on a laptop, and `HOSTING.md`'s B1ms moves **10 MiB/s**, which makes the same scan **~508 s** against `deploy.yml`'s `timeout 120`. Every migration on that table is metadata-only at deploy time; a rewrite, a validated constraint or a non-concurrent index is measured against 10 MiB/s before it is written, and a check that only guards new rows stays `NOT VALID` on purpose. **And metadata-only is not the same as free — added 2026-09-23 by Task 3.7.6.** `ALTER TABLE` takes an `ACCESS EXCLUSIVE` lock whatever it costs, Postgres queues later requests **behind** a waiting exclusive one, and neither `lock_timeout` nor `statement_timeout` is set anywhere in this repository. Rehearsed on the populated store: a migration waited **18.16 s** behind one open write transaction, and an ordinary chart read — whose own lock conflicts with nothing that was granted — waited **16.16 s** behind _it_, against a 0.09 s baseline. So the question to ask of a migration on this table is not only _what does it cost_ but _what is open when it runs_, and the answer that bounds it today is that one `recordSeries` transaction is 390 rows and ~100 ms. `docs/GAPS.md` carries the figures, the `lock_timeout` cure and why it was not bought. **And an index it cannot build at all — added 2026-09-23 by Task 3.8.2.** `CREATE INDEX CONCURRENTLY` cannot run inside Kysely's single transaction and a non-concurrent build does not fit 120 s (37.3 s on a laptop, ~508 s a heap pass on the tier), so an index on this table is built by **`pnpm index:prepare`**, its own deploy step, and the migration only **adopts** it — 0.6 s measured, against 30.7 s for a migration that builds it inline. `apps/backend/migrations/README.md` §9 is the convention. Its corollary is the part that bit: **when a key gains a column, grep for every query that assumed the old one** — `writeBatch`'s presence check was not scoped to the new column and would have made the ledger under-report silently.
...
- **`vi.unstubAllGlobals()` removes stubs installed by `setupFiles`, not just the caller's own.** Vitest keeps one stub registry per test file and that call empties it — so a global the setup file installed for the whole suite is handed back to every test _after_ the first teardown that runs it. The frontend's no-socket guarantee hit this: a `vi.stubGlobal("WebSocket", …)` in `test-setup.ts` held for some files and silently lapsed in the ones that stub `fetch`. **A global the suite relies on is assigned directly** (`globalThis.X = …`), which is outside the registry; `vi.stubGlobal` is for a stub one test owns.
- **An unhandled error is not a failed assertion, and the run can still exit 0.** `App` gained a real `WebSocket` in Story 3.3 and every test rendering it dialled `localhost:3000`; Vitest reported `Unhandled Errors`, attributed them to whichever test was running when they landed, and **passed for two tasks** until a timing change made the run fail. `pnpm test`'s "no socket, no network" is a contract, and a contract nothing enforces expires quietly — read the `Unhandled Errors` block, never only the exit code.
