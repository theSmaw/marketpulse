...
**Both are skeletons in scope, not in status.** The backend has no market data, no database, no domain logic, no configuration module and no CORS. The frontend has no routing (Story 1.5), no styling system (Story 1.4), no state management, and does not call the backend (Story 1.12). Neither has tests (Story 1.9). Stories 1.4–1.12 can all proceed from here.
...
**The prediction that Vite would fire it was wrong, and `allowBuilds` is still empty and still untested.** esbuild was the expected first trip; Vite 8 is the Rolldown release, so there is **no esbuild in this toolchain at all**, and Rolldown ships as prebuilt per-platform binaries. Four sweeps — Tasks 1.3.1, 1.3.2, 1.3.3, and a cold 200-package install in 1.3.5 — found zero `preinstall`/`install`/`postinstall` scripts anywhere in the tree. Write it as a resolved wrong prediction rather than a standing expectation: the policy holds, but nobody has seen it fire.
...
The apps override exactly **six** compiler options between them — it was four until Story 1.3 — and each is load-bearing. The backend sets `types: ["node"]` (with `@types/node` pinned to the runtime major, 24.x — not npm's `latest`). The frontend sets `types: []` plus `target`/`lib` with `"dom"`, and adds two more:
...
The frontend's empty `types` array is not redundant: without it TypeScript auto-discovers every reachable `@types` package, and pnpm's linking puts `@types/node` in reach, so `process` would typecheck in browser code. **Adding `@types/react` did not change it** — React's types arrive through `import` statements rather than global auto-inclusion, so the array is for _globals_ and React has none. Adding an entry there is the wrong reflex for any library that is imported rather than ambient.
